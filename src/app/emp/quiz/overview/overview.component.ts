...
export class OverviewComponent implements OnInit {
  constructor(private api: EmpapiService, private toastController: ToastController,public authService: AuthService,public router:Router,private title: Title,
    ...
    this.title.setTitle("Quiz Overview - "+environment.companyName);
   }
  ...
  expiredToast(){
    this.errorPresentToast("Quiz is expired");
  }

  getData(){
    this.api.quizCountForQuiz(this.authService.currentUserValue.id).subscribe(data=>{
      this.quizLength = data.quizLength;
        this.completedQuizLength = data.completedLength;
    })
    this.api.getQuizCompletionStatusByEmp(this.authService.currentUserValue.id).subscribe(data=>{
  ...
  filerData(value:any) {
    ...
    this.programData = this.programData.filter((currentGoal:any) => {
      ...
      if (currentGoal.quizDetails[0].title && value) {
        if (currentGoal.quizDetails[0].title.toLowerCase().indexOf(value.toLowerCase()) > -1) {
          return true;
        }
        return false;
      }
